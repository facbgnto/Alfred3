...
Roo Code carga además `.roo/rules/*.md` (capa fina que referencia este
archivo, sin duplicar contenido).
...
- Skills peligrosas (shell, instalaciones, push, acciones administrativas) no
  se ejecutan sin confirmación persistente explícita del usuario.
- El frontend nunca llama `voice-service` (127.0.0.1:8765) directo; todo pasa
  por el gateway `apps/api` (`/api/voice/*`).
...
- Sistema hoy es single-user local. Si se agrega multi-tenant, defínelo
  primero en un ADR (`docs/adr/`) — no lo asumas ni inventes campo tenant.
